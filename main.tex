...
Come si può vedere dalla Figura \ref{fig:PitCoverageReadHeader1} la mutation coverage è del 3\% in riferimento all'intera classe.
...
\[\frac{{Mutazioni rilevate}}{{Mutazioni totali}} = \frac{{5}}{{17}} = 0.29\]

Prendiamo in considerazione per migliorare la test suite le due mutazione non rilveate a riga 242. 
Le due mutazioni (Figura \ref{fig:PitMutationReadHeader3}) vanno a modificare le condizioni dell'if. La prima trasforma 
il \textgreater= in \textgreater, la seconda invece trasforma il \textgreater= in \textless.

Vado quindi a modificare la test suite aggiungendo dei casi di test che coprano queste due mutazioni.
Per indentificarle basta notare che solo nel caso in cui la versione sia >= 1 l'explicitLacBufLength
deve essere configurato nell'header, negli altri casi no. 
Per fare ciò ho modificato un caso di test in modo da controllare se l'eccezione chiamata avveniva solo nel caso in cui
la condizione dell'if fosse vera, e quindi la mutazione ora viene rilevata. (Figura \ref{fig:PitMutationReadHeader4})
