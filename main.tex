...
\begin{itemize}
  \item \{ = len master key\} = len master key
  \item \{< len master key\} = len master key - 1
  \item \{> len master key\} = len master key + 1
\end{itemize}
...
Come si può vedere della figura \ref{fig:JacocoCoveragereadHeader1}
...
\begin{table}[ht]
  ...
  \begin{tabular}{|l|l|l|l|}
    \hline
    \textbf{Magic Bytes} & \textbf{Master key} & \textbf{Len of master key} & \textbf{Risultato Atteso} \\
    \hline
    BKLE &  & new byte[] &  1 \\
    \hline
  \end{tabular}
